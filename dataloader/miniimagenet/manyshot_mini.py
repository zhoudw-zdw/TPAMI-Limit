...
class MiniImageNet(Dataset):
    #这个数据集要check一下，参数可能写的有问题
    def __init__(self, root='/data/zhoudw/FSCIL', train=True,
                 transform=None,
                 index_path=None, index=None, base_sess=None,autoaug=0,shotnumber=5):
        if train:
            setname = 'train'
        else:
            setname = 'test'
        ...
        if train:
            ...
            if base_sess:
                ...
                self.data, self.targets = self.SelectfromClasses_withnum(self.data, self.targets, index,shotnumber)
        else:
            image_size = 84
            self.transform = transforms.Compose([
    ...
    def SelectfromClasses_withnum(self, data, targets, index,num):
        data_tmp = []
        targets_tmp = []
        print(index)
        for i in index:
            ...
            #     data_tmp.append(data[j])
            #     targets_tmp.append(targets[j])
            for j in range(min(len(ind_cl),num)):
                data_tmp.append(data[ind_cl[j]])
                targets_tmp.append(targets[ind_cl[j]])
