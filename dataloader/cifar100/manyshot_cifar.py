...
class CIFAR10(VisionDataset):
    ...
    def __init__(self, root, train=True, transform=None, target_transform=None,
                 download=False, index=None, base_sess=None,autoaug=0, shotnumber=0):
        ...
        if base_sess:
            ...
            if train:
                self.data, self.targets = self.NewClassSelector(self.data, self.targets, index,shotnumber)
            else:
                self.data, self.targets = self.SelectfromDefault(self.data, self.targets, index)
    ...
    def NewClassSelector(self, data, targets, index,shotnumber):
        ...
        for i in index:
            ind_cl = np.where(i == targets)[0]
            if data_tmp == []:
                data_tmp = data[ind_cl][:shotnumber]
                targets_tmp = targets[ind_cl][:shotnumber]
            else:
                data_tmp = np.vstack((data_tmp, data[ind_cl][:shotnumber]))
                targets_tmp = np.hstack((targets_tmp, targets[ind_cl][:shotnumber]))
        assert (len(data_tmp)==len(targets_tmp))
        #print(len(data_tmp),len(targets_tmp))
...
class CIFAR_tensordataloader(VisionDataset):
    
    def __init__(self, root=None, transform=None, target_transform=None,autoaug=0,x=None,y=None):

        super(CIFAR_tensordataloader, self).__init__(root, transform=transform,
                                      target_transform=target_transform)
        self.train=True
        if autoaug==0:
            if self.train:
